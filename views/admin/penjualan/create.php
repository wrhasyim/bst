<div class="max-w-2xl mx-auto space-y-8 pb-10">
    <div class="flex justify-between items-end">
        <div>
            ...
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Input Data Penjualan & Negosiasi Harga (Sistem KG - Pembulatan)</p>
        </div>
    ...
    <div class="bg-white rounded-[3rem] border border-slate-200 p-8 shadow-sm"
         x-data="{ 
             ...
             get konversi() {
                 return this.detail && this.detail.konversi_kg ? parseInt(this.detail.konversi_kg) : 1;
             },
             get totalKg() {
                 let kg = Math.round(this.jumlah / this.konversi);
                 return kg > 0 ? kg : (this.jumlah > 0 ? 1 : 0);
             }
         }">
        ...
        <form action="<?= BASE_URL ?>/penjualan/store" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="kategori_id" x-model="katId" required 
                        @change="harga = detail ? parseFloat(detail.harga_pengepul) : 0; jumlah = detail ? parseFloat(detail.stok_tersedia) : 0"
                    ...
                    <template x-for="k in list" :key="k.id">
                        <option :value="k.id" x-text="k.nama_sampah + ' (Tersedia: ' + k.stok_tersedia + ' Pcs)'"></option>
                    </template>
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1 tracking-widest">2. Total Stok Jual (Pcs)</label>
                    <div class="relative">
                        <input type="number" step="1" name="total_pcs" x-model.number="jumlah" readonly 
                               class="w-full px-5 py-4 bg-slate-100 border border-slate-200 rounded-2xl font-black text-xl outline-none transition-all text-slate-500 cursor-not-allowed">
                        <span class="absolute right-5 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-400 uppercase">Pcs</span>
                    </div>
                    <p class="text-[10px] text-emerald-600 font-bold mt-2 ml-1 italic" x-show="katId">
                        ⚖️ Setara : <span x-text="totalKg.toLocaleString('id-ID')"></span> KG (Dibulatkan)
                        <span class="text-slate-400 ml-1 block mt-1">(1 KG = <span x-text="konversi"></span> Pcs)</span>
                    </p>
                </div>
                <div>
                    ...
                    <p class="text-[9px] text-slate-400 font-bold mt-2 ml-1 uppercase italic">*Harga kesepakatan per Kilogram (KG).</p>
                </div>
            </div>

            <div x-show="jumlah * harga > 0 && katId" x-collapse
                ...
                <div>
                    ...
                    <span class="block text-3xl font-black text-emerald-400 tracking-tighter">Rp <span x-text="new Intl.NumberFormat('id-ID').format(totalKg * harga)"></span></span>
                </div>
            ...
            <button type="submit" 
                    :disabled="jumlah <= 0 || !katId"
                    :class="jumlah <= 0 || !katId ? 'opacity-30 cursor-not-allowed bg-slate-300' : 'bg-emerald-600 hover:bg-emerald-700 shadow-xl shadow-emerald-500/20'"
